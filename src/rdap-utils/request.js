...
function redirectToHref(loc) {
    ...
    if (m) {
        ...
        return leftStr + m[0].replace(m[1], '') + encodeURIComponent(
            window.location.href) + rightStr
    }
    return loc
}
...
export function request(url, options, useQueue = false) {
    ...
    if (useQueue) {
      return _addFetchQueue(lastUrl, lastOption).then(dealStatus).then(parseJson)
    }else {
      return fetch(lastUrl, lastOption).then(dealStatus).then(parseJson)
    }
}
...
export function getUrlParam(search, name) {
    ...
    if (!r && window.location.hash && window.location.hash.indexOf('?') > 0) {
        r = window.location.hash.substr(window.location.hash.indexOf('?') +
            1).match(reg)
    }
